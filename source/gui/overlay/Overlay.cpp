        ...
        while (!glfwWindowShouldClose(m_windowHandle))
        {
            ...
            if (localPlayer.has_value())
                for (auto const entity : apex_sdk::EntityList::GetAllEntities())
                {
                    ...
                    auto top = apex_sdk::WorldToScreen(entity.GetBonePosition(apex_sdk::BoneIndex::Neck)+uml::Vector3{0.f,0.f,10.f},
                                                       1920,
                                                       1080);
                    ...
                    drawList->AddLine(topLeft.As<ImVec2>(), topRight.As<ImVec2>(), ImColor(255, 0,0));
                    drawList->AddLine(bottomLeft.As<ImVec2>(), bottomRight.As<ImVec2>(), ImColor(255, 0,0));
                    drawList->AddLine(topLeft.As<ImVec2>(), bottomLeft.As<ImVec2>(), ImColor(255, 0,0));
                    drawList->AddLine(topRight.As<ImVec2>(), bottomRight.As<ImVec2>(), ImColor(255, 0,0));

                    const auto entShieldsMax = entity.GetMaxShieldValue();
                    if (entShieldsMax > 0 && entity.GetShieldValue() > 0)
                        drawList->AddText({topRight.x+5, topRight.y},
                                          entity.GetShieldColor().As<ImColor>(),
